...
export default function FontsView() {
  ...
  const handleUpdate = () => {
    if (editRow !== null) {
      setSavedFonts((prev) =>
        prev.map((row, idx) => (idx === editRow ? { fontFiles: [...fontFiles], sizes: { ...sizes } } : row)),
      )
      setEditRow(null)
      setFontFiles([null, null])
      setSizes({ paragraph: 16, subtitle: 24, title: 32 })
      ...
      <div className="min-h-screen bg-gray-50 py-6 flex flex-col justify-center sm:py-12">
        <div className="relative py-3 sm:max-w-xl sm:mx-auto">
          <div className="relative px-4 py-10 bg-white border border-gray-200 shadow-lg rounded-lg sm:p-12">
            <h1 className="text-2xl font-semibold text-[#005CA1] text-center mb-6">
            ...
            <div className="mt-6 flex flex-col gap-6">
              <div className="flex flex-col gap-4">
                {[0, 1].map((idx) => (
                  <div className="flex items-center space-x-4" key={idx}>
                    <label htmlFor={`font-file-${idx}`} className="text-gray-700 font-medium">
                      Fuente {idx + 1}:
                    </label>
                    <input
                      id={`font-file-${idx}`}
                      ...
                      className="file:bg-[#005CA1] file:hover:bg-[#004080] file:text-white file:font-medium file:rounded file:border-none file:px-3 file:py-2 bg-white border border-gray-300 text-gray-700 cursor-pointer w-56 transition-colors"
                    />
                    {fontFiles[idx] && <span className="text-xs text-gray-600">{fontFiles[idx]?.name}</span>}
                  </div>
                ))}
              </div>
              <div className="flex flex-col gap-2 mt-6">
                <div className="flex items-center space-x-2">
                  <div className="flex justify-between items-center w-[40%]">
                    <label htmlFor="size-paragraph" className="text-gray-700 text-sm">
                      Párrafo:
                    </label>
                    <input
                      id="size-paragraph"
                      ...
                      className="w-16 px-2 py-1 rounded bg-white border border-gray-300 text-gray-700 focus:border-[#005CA1] focus:outline-none"
                    />
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="flex justify-between items-center w-[40%]">
                    <label htmlFor="size-subtitle" className="text-gray-700 text-sm">
                      Subtítulo:
                    </label>
                    <input
                      id="size-subtitle"
                ...
                <div className="flex items-center space-x-2">
                  <div className="flex justify-between items-center w-[40%]">
                    <label htmlFor="size-title" className="text-gray-700 text-sm">
                      Título:
                    </label>
                    <input
                      id="size-title"
              ...
              <div className="flex justify-between mt-8 gap-4">
                ...
                <Link
                  to="/"
                  className="bg-white hover:bg-gray-50 text-[#005CA1] border border-[#005CA1] font-medium py-2 px-4 rounded transition-colors"
                >
                  Regresar
                </Link>
                <Link
          ...
          <div className="relative px-4 py-10 bg-white border border-gray-200 shadow-lg rounded-lg sm:p-12 w-full">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead>
                  <tr>
                    <th className="px-2 py-2 text-[#005CA1] border-b border-gray-200">Fuente 1</th>
                    <th className="px-2 py-2 text-[#005CA1] border-b border-gray-200">Fuente 2</th>
                    <th className="px-2 py-2 text-[#005CA1] border-b border-gray-200">Párrafo</th>
                    <th className="px-2 py-2 text-[#005CA1] border-b border-gray-200">Subtítulo</th>
                    <th className="px-2 py-2 text-[#005CA1] border-b border-gray-200">Título</th>
                      ...
                      {row.fontFiles.map((file, idx) => (
                        <td key={idx} className="px-2 py-2 text-center">
                          {file ? (
                            ...
                            <span className="text-xs text-gray-400">No seleccionado</span>
                          )}
                        </td>
                      ))}
                      <td className="px-2 py-2 text-center text-gray-600">{row.sizes.paragraph}</td>
                      <td className="px-2 py-2 text-center text-gray-600">{row.sizes.subtitle}</td>
                      <td className="px-2 py-2 text-center text-gray-600">{row.sizes.title}</td>
